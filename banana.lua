		...
		while isScriptActive and currentMode == "Banana" do
			...
			if #targets > 0 then
				for _, target in ipairs(targets) do
					...
					-- === ГЛАВНАЯ ПРОВЕРКА: ОН ВСЕ ЕЩЕ RUNNER? ===
					if not target.Team or target.Team.Name ~= RUNNER_TEAM then
						print("Пропуск: " .. target.Name .. " больше не Runner (Лобби)")
						continue -- Пропускаем этого игрока, не телепортируемся
					end
					...
					statusLabel.Text = "🍌 Атака: " .. target.Name
					...
					while tick() - startTime < 2 do
						...
						-- Постоянная проверка команды во время атаки
						if target.Team.Name ~= RUNNER_TEAM then break end
...
local function OnTeamChanged()
	...
	if teamName == "Lobby" or teamName == "Spectators" or teamName == "None" then
		currentMode = "Lobby"
		CleanUpPhysics()
		statusLabel.Text = "В Лобби (AFK + Bonus)"
		statusLabel.TextColor3 = Color3.fromRGB(200, 200, 200)
		SetupBonusListener()
...
RunService.Stepped:Connect(function()
	...
	if currentMode == "Runner" and gameClock and gameClock.Value <= 60 and gameClock.Value > 50 and not hasEscaped then
		...
		if exitsFolder then
			...
			if primaryExit then targetPart = primaryExit:FindFirstChild("Neon") or primaryExit.PrimaryPart end
			
			if targetPart then
				statusLabel.Text = "🏃 ПОБЕГ: ВЫХОД 2"
				statusLabel.TextColor3 = Color3.fromRGB(0, 255, 255)
				task.spawn(function() SpamTeleport(targetPart.CFrame + Vector3.new(0, 3, 0), 3) end)
			end
			...
			task.delay(13, function()
				if isScriptActive and player.Team and player.Team.Name == RUNNER_TEAM then
					...
					if backupExit then
						...
						if backupPart then SpamTeleport(backupPart.CFrame + Vector3.new(0, 3, 0), 5) end
					end
				end
			end)
		end
